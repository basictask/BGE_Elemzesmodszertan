...
df_pred['2var_tree'] = clf_tree_2var.predict(x_val)
...
bag_clf.fit(x_train[['Hossz1','Magassag']], y_train)
y_pred_bag = bag_clf.predict(x_val[['Hossz1','Magassag']])

df_pred['Bagging'] = y_pred_bag
...
#%% Legjobb döntési fa megtalálása
...
y_pred_ensemble = np.empty([n_trees, len(x_val)], dtype='O')

for tree_index, tree in enumerate(forest):
    y_pred_ensemble[tree_index] = tree.predict(x_val)
    
y_pred_majority_votes, n_votes = mode(y_pred_ensemble, axis=0)

ensemble_preds = pd.Series(y_pred_majority_votes[0], index=df_pred.index)

df_pred['Ensemble'] = ensemble_preds
...
df['y'] = list(load_breast_cancer().target)
df.head()
...
df = df.drop(list(corr['y'][abs(corr['y'])<0.5].index),axis=1)
...
A = [experiment('binary', 'binary', x_train, x_test, y_train, y_test, k) for k in range(10)]

second_target = 'tweedie'
tree_num = 10
B = [experiment(second_target, 'binary', x_train, x_test, y_train, y_test, k) for k in range(tree_num)]
...
def accplot(accdata, objective):
    ...
    ax1.set_title('binaris pontossag')
    ax2.plot(accuracy_mean(accdata))
    ax2.set_title('valoszinusegi rezidumok')
    plt.xticks(np.arange(0,len(accdata)))
    plt.show()

accplot(A, 'binary')
accplot(B, second_target)


#%% Kontingencia tábla
y_predfinal = pd.DataFrame({'pred': A[len(A)-1]['y_pred'].copy().round()})
confusion_matrix = confusion_matrix(y_test, y_predfinal)
print(classification_report(y_test, y_predfinal))

confusion_matrix_df=pd.DataFrame(confusion_matrix,('No cancer', 'Cancer'),('No cancer', 'Cancer'))
...
def animation_frame(i):
    line.set_xdata(np.arange(0,len(C[i])))
    fig.suptitle('Logisztikus regresszió bináris célponttal (' + str(i+1)+' fa)', fontsize=20)
    y_data = []
    line.set_ydata(C[i]['y_pred'])
    ax.plot(C[0]['y'],'o', color='black')
    return line,

animation = FuncAnimation(fig, func=animation_frame, frames=range(10), interval=200)
plt.show()
...
def animation_frame(i):
    line.set_xdata(np.arange(0,len(C[i])))
    fig.suptitle('GBDT Logisztikus regresszió '+second_target+' célponttal (' + str(i+1)+' fa)',fontsize=20)
    y_data = []
    line.set_ydata(C[i]['y_pred'])
    ax.plot(C[i]['y'],'o', color='black')
    return line,
