...
model.add(Dense(10, activation="softmax", name='preds')) ##eloszlasi valoszinuseg minden osztalyra

model.summary()


#%%10 Modell kirajzolása
tensorflow.keras.utils.plot_model(model, "my_fashion_mnist_model.png", show_shapes=True)
...
keract_inputs = X_train[3:4] ##ez egy 4-es szam
keract_targets = Y_train[3:4]
...
keract_targets = Y_train[3:4]
activations = get_activations(model, keract_inputs)
display_heatmaps(activations, keract_inputs, fix=False)


#%%14 Adataugmentáció nélkül
history = model.fit(X_train, Y_train, batch_size=433, epochs=10, 
...
#%%20 Normalizlt konfúziós mátrix
